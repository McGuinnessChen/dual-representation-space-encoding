...
class OmniglotDatasetForSampling:
  ...
  def _load_omniglot_data(self, split, exemplars):
    ...
    if split == 'all':
      ...
      ds = tfds.load(
          'omniglot', split=split, as_supervised=True, shuffle_files=False, try_gcs=False, data_dir="emergent_in_context_learning/tfds")
      data = {}
...
class LlamaEmbeddingDatasetForSampling:
  """Class for loading precomputed LLaMA embeddings from an HDF5 file."""

  def __init__(self, embedding_path, dataset_path, dtype=np.float32):
    """Load LLaMA embeddings into memory.

    Args:
      embedding_path: Path to the HDF5 file containing embeddings.
      dataset_path: Path inside the HDF5 file where the embeddings live.
        The dataset is expected to have shape [n_classes, n_exemplars, emb_dim].
      dtype: Data type to cast the embeddings to.
    """
    with h5py.File(embedding_path, 'r') as f:
      data = np.array(f[dataset_path], dtype=dtype)

    # Create dictionary mapping each class to its list of exemplar embeddings.
    self.data = {i: list(class_embeds) for i, class_embeds in enumerate(data)}
    self.example_type = 'llama3'
    self.embedding_dim = data.shape[-1]
...
class SeqGenerator:
  ...
  def __init__(self,
    ...
    # Capture example shape and dtype for constructing sequence arrays.
    sample_example = self._get_sample_example()
    self.example_shape = sample_example.shape
    self.example_dtype = sample_example.dtype
  ...
  def _get_sample_example(self):
      example = self.data[self.classes[0]]
      if isinstance(example, list):
          example = example[0]
      return np.asarray(example)

  def _init_seq_array(self, seq_len):
      return np.zeros((seq_len,) + self.example_shape, dtype=self.example_dtype)

  def _sample_examples_for_classes(self, classes):
      classes = np.array(classes)
      if isinstance(self.data[classes[0]], list):
          examples = self._init_seq_array(len(classes))
          unique_classes = np.unique(classes)
          for c in unique_classes:
              sample_count = np.sum(classes == c)
              replace = sample_count > len(self.data[c])
              c_samples = np.random.choice(
                  len(self.data[c]), size=sample_count, replace=replace)
              examples[classes == c] = np.array(self.data[c])[c_samples]
      else:
          examples = np.array([self.data[c] for c in classes],
  ...
  def _create_noisy_image_seq(self,
    ...
    classes = np.array(classes)
    if randomly_generate_rare and self.example_type == 'omniglot':
      ...
      examples = self._sample_examples_for_classes(classes)

    # Add pixel noise to the images.
    if self.noise_scale and self.example_type == 'omniglot':
      noise = np.random.normal(0, self.noise_scale, examples.shape)
      examples += noise.astype(examples.dtype)
  ...
  def get_no_support_seq(self,
    ...
    while True:
      ...
      seq_examples = self._init_seq_array(seq_len)
