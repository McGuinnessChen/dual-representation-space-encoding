...
class CoQEModel(nn.Module):
    def __init__(self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4):
        super(CoQEModel, self).__init__()
        configuration = GPT2Config(
            n_positions=2 * n_positions,
            n_embd=n_embd,
            n_layer=n_layer,
        ...
        self.name = f"coqe_gpt2_embd={n_embd}_layer={n_layer}_head={n_head}"
    ...
    def forward(self, xs, ys, inds=None):
        ...
        embeds = self._read_in(zs)
        output = self._backbone(inputs_embeds=embeds).last_hidden_state
        output = self._read_out(output)
        output = torch.roll(output, shifts=1, dims=1)

        prediction = (embeds * output).sum(dim=2, keepdim=True)
        return prediction[:, ::2, 0][:, inds]  # predict only on xs
