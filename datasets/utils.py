...
def prepare_seqs_for_transformer(ds,
  ...
  def _convert_dict(example):
    ...
    example_tensor = example['example']
    is_image = (len(example_tensor.shape) == 5)
    is_float = example_tensor.dtype.is_floating

    # Cast the examples into the correct shape and tf datatype.
    if is_image:
      # examples = tf.cast(example['example'], tf.float32)  # (B,SS,H,W,C)
      examples = tf.cast(example_tensor, tf.float32)  # (B,SS,H,W,C)
      if downsample:
        examples = tf.map_fn(lambda batch: tf.image.resize(batch, [28, 28]),
                             examples)
    elif is_float:
      examples = tf.cast(example_tensor, tf.float32)
    else:
      examples = tf.cast(example_tensor, tf.int32)
