...
def eval_batch(model, task_sampler, xs, xs_p=None):
    task = task_sampler()
    if torch.cuda.is_available() and model.name.split("_")[0] in ["gpt2", "coqe", "lstm"]:
        device = "cuda"
    else:
        device = "cpu"
...
def gen_ood_x(data_sampler, n_points, b_size):
    xs = data_sampler.sample_xs(n_points, b_size)
    xs_ood = 3 * xs
    return xs, xs_ood
...
def get_run_metrics(
    run_path, base_path=None, step=-1, cache=True, skip_model_load=False, skip_baselines=False
): # Used to fairly compare two trained models on the same task (with checkpoints located at `run_path` and `base_path`, respectively).
    if skip_model_load:
        ...
        if base_path is not None:
            base_model, base_conf = get_model_from_run(base_path, step)
    ...
    #     save_path = os.path.join(run_path, "metrics_result.json")
    elif step == -1:
...
def conf_to_model_name(conf):
    ...
    elif conf.model.family == "coqe_gpt2":
        return "CoQE"
    else:
...
if __name__ == "__main__":
    get_run_metrics("..\\relu_2nn_regression\\0827192358","..\\relu_2nn_regression\\0827192157")
