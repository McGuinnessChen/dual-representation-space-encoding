  ...
  def __call__(self, examples, labels, is_training=True):
    ...
    if self._example_encoding == 'resnet':
      ...
      example_encoding = resnet.SimpleResNet(
          blocks_per_group=(2, 2, 2, 2), # (2, 2, 2, 2)
          # channels_per_group=(64, 128, 256, resnet_emb_dim),
          # channels_per_group=(64, 128, 128, resnet_emb_dim),
    ...
    elif self._example_encoding == 'llama3':
      example_mlp = hk.nets.MLP(
        [512, 256, self._emb_dim],
        activation=jax.nn.relu,
        activate_final=False,
      )
      example_encoding = hk.BatchApply(example_mlp)
      h_example = example_encoding(examples)
    else:
      raise ValueError('Invalid example_encoding: %s' % self._example_encoding)

    # if is_training:
    #   # 1. 生成掩码: 哪些位置需要被替换 (True为替换，False为保留)
    #   # Bernoulli分布生成 0 或 1
    #   mask_key, noise_key = jax.random.split(hk.next_rng_key())
    #   noise_mask = jax.random.bernoulli(mask_key,
    #                                     p=0.1,
    #                                     shape=h_example.shape)
    #
    #   # 2. 生成噪声数据: 这里使用标准正态分布 N(0, 1)
    #   # 如果你的 embedding 数值范围很大或很小，可能需要调整 stddev
    #   random_noise = jax.random.normal(noise_key, shape=h_example.shape)
    #
    #   # 3. 执行替换: 如果 mask 是 True，取 noise；否则保持 h_example 原值
    #   h_example = jnp.where(noise_mask, random_noise, h_example)
